...
export const detectStoreSegment = (store: StoreLike, menuCategories: string[] = []): StoreSegment => {
  ...
  if (/(acai|cupacu|smoothie|sorvete|gelato|milk shake|milkshake)/.test(text)) return "Açaí";
  ...
  if (/(farmacia|drogaria|medicamento|saude|perfumaria)/.test(text)) return "Farmácia";
...
export const getSegmentCover = (segment: StoreSegment | string) => {
  ...
  if (normalized === "acai") return SEGMENT_COVERS["Açaí"];
  if (normalized === "farmacia") return SEGMENT_COVERS["Farmácia"];
  return SEGMENT_COVERS[(segment as StoreSegment) in SEGMENT_COVERS ? (segment as StoreSegment) : "Restaurantes"];
};
