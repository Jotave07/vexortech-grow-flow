...
import { Loader2, Search, ExternalLink, Pause, Play, Trash2, ShieldCheck, Plus, Store as StoreIcon, CheckCircle2, Layers, MapPin, Pencil } from "lucide-react";
...
const AdminStores = () => {
  ...
  const load = async () => {
    ...
    const [{ data: s }, { data: p }] = await Promise.all([
      backend.from("stores").select("*, subscriptions(status, plan_id, asaas_subscription_id, billing_type, plans(name, price_monthly))").order("created_at", { ascending: false }),
  ...
  const filtered = useMemo(() => {
    ...
    return stores.filter((s) => {
      if (q) {
        const matches = s.name.toLowerCase().includes(q) || s.slug.toLowerCase().includes(q) || s.email?.toLowerCase().includes(q);
  ...
  const changePlan = async (store: any, planId: string) => {
    if (planId === "cortesia") {
      ...
      try {
        ...
        await backend.from("profiles").update({ is_exempt: true }).eq("id", store.owner_profile_id);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-3xl font-black uppercase tracking-tight">Lojas</h1>
          <p className="font-medium text-muted-foreground">Gestão de parceiros e lojas da plataforma.</p>
        </div>
        <Button variant="hero" className="h-10 font-black uppercase tracking-widest text-xs" onClick={() => setCreateModalOpen(true)}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <SummaryTile label="Total de Lojas" value={String(summary.total)} icon={StoreIcon} tone="primary" />
        <SummaryTile label="Ativas" value={String(summary.active)} icon={CheckCircle2} tone="info" />
      ...
      <Card className="p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1">
            ...
            <Input className="pl-9" placeholder="Buscar por nome, slug ou e-mail..." value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
      ...
      {filtered.length === 0 && (
        <Card className="p-12 text-center">
          ...
          <p className="text-sm italic text-muted-foreground">Nenhuma loja encontrada com os filtros atuais.</p>
        </Card>
      )}
          ...
          {selected && (
            <div className="space-y-4">
              <div className="rounded-none border border-border bg-muted/30 p-4 text-xs text-muted-foreground space-y-1.5">
                ...
                <div>Cidade: <span className="text-foreground">{selected.city ?? "—"} {selected.state && `/${selected.state}`}</span></div>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground">Plano de Referência</label>
                <Select value={selected.is_exempt ? 'cortesia' : selected.subscriptions?.[0]?.plan_id || (selected.subscriptions?.[0]?.status === 'ativa' && !selected.subscriptions?.[0]?.plan_id ? 'cortesia' : '')} onValueChange={(v) => changePlan(selected, v)}>
              ...
              <div className="flex flex-col gap-2 pt-2">
                <Button asChild variant="outline" className="font-bold uppercase tracking-widest text-xs">
                ...
                <Button variant={selected.is_suspended ? "hero" : "outline"} className="font-bold uppercase tracking-widest text-xs" onClick={() => toggleSuspend(selected)}>
                  {selected.is_suspended ? <><Play className="h-4 w-4" /> Reativar</> : <><Pause className="h-4 w-4" /> Suspender</>}
                </Button>
                <Button variant="destructive" className="font-bold uppercase tracking-widest text-xs" onClick={() => remove(selected)}><Trash2 className="h-4 w-4" /> Excluir loja</Button>
              </div>
            </div>
          )}
        </DialogContent>
      ...
      <Dialog open={createModalOpen} onOpenChange={setCreateModalOpen}>
        <DialogContent className="max-w-2xl overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="font-black uppercase tracking-tight">Cadastrar Nova Loja e Empresário</DialogTitle>
          </DialogHeader>
...
const StoreStatusBadge = ({ store }: { store: any }) => {
  if (store.is_suspended) {
    return <Badge variant="secondary" className="bg-muted text-muted-foreground border-border text-[10px] font-bold uppercase tracking-widest">Fechada</Badge>;
  }
  if (!store.is_active) {
    return <Badge variant="outline" className="bg-amber-50 text-amber-600 border-amber-200 text-[10px] font-bold uppercase tracking-widest">Em análise</Badge>;
  }
  return <Badge variant="outline" className="bg-primary/10 text-[var(--hype-green-dark)] border-primary/30 text-[10px] font-bold uppercase tracking-widest">Aberta</Badge>;
};
    ...
    <Card className="group flex flex-col overflow-hidden p-0 transition-smooth hover:border-primary">
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-1.5">
            ...
            {store.is_exempt && (
              <Badge variant="outline" className="bg-primary/10 text-[var(--hype-green-dark)] border-primary/30 text-[10px] font-bold uppercase tracking-widest">
      ...
      <div className="grid grid-cols-2 gap-px border-t border-border bg-border">
        <div className="bg-card p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Plano</div>
          <div className="truncate text-sm font-black">{planName}</div>
        </div>
        <div className="bg-card p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Mensalidade</div>
          <div className="truncate text-sm font-black">{sub?.plans?.price_monthly ? formatBRL(sub.plans.price_monthly) : "—"}</div>
        </div>
        <div className="bg-card p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Local</div>
        ...
        <div className="bg-card p-3">
          <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Cadastro</div>
          <div className="truncate text-sm font-black">{new Date(store.created_at).toLocaleDateString("pt-BR")}</div>
        </div>
      </div>

      {/* Ações */}
      <div className="mt-auto flex items-center gap-2 border-t border-border bg-muted/30 p-3">
        <Button asChild variant="outline" size="sm" className="flex-1 font-bold uppercase tracking-widest text-[10px]">
          <a href={buildDeliveryUrl(`/loja/${store.slug}`)} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()}>
            <ExternalLink className="h-3.5 w-3.5" /> Ver
          </a>
        </Button>
        <Button variant="hero" size="sm" className="flex-1 font-bold uppercase tracking-widest text-[10px]" onClick={onOpen}>
