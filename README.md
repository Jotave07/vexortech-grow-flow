...
Em produção, a VPS/Node entrega o frontend e o health check; o Nginx encaminha o BFF, o webhook Asaas e o acesso intermediado a arquivos para Supabase Edge Functions. PostgreSQL, Auth, Storage e Broadcast privado usam o mesmo projeto Supabase. O handler compartilhado continua disponível no Node como fallback imediato, com Asaas, Evolution e Google Maps configurados somente no servidor.

## Estado da modernização

Revisão deste estado: **2026-07-14**.
...
- Dependências diretas de produção reduzidas para `pg`, `zod` e o cliente pinado `@supabase/realtime-js`.
- Legado e resíduos inventariados antes da remoção e preservados em quarentena local verificável.
- O gate consolidado de cada release executa typecheck, lint, testes, build e E2E sobre o mesmo HEAD limpo que gera o artefato.
- Contagens de testes registradas em relatórios anteriores são snapshots históricos e não substituem o gate da release atual.
- A revisão documental de 2026-07-14 não afirma que migrations foram aplicadas no banco remoto, que integrações reais foram homologadas ou que o deploy atual já ocorreu.
...
- Projeto Supabase e PostgreSQL acessíveis.
- Variáveis de ambiente descritas em `.env.example`.
- Para produção, Supabase CLI `2.109.1` autenticada e um token de Management API protegido com permissões de leitura/escrita da configuração Realtime.
...
- `.output/server/vanilla-server.js`: servidor Node compilado para estáticos, health e fallback do BFF.
...
O servidor escuta `PORT=3000` por padrão. Em produção, mantenha `HOST=127.0.0.1` e publique a aplicação por Nginx; as rotas exatas de backend usam o snippet ativo Node/Edge descrito no runbook.
...
Desde então, o histórico continuou evoluindo. `db:migrate` aplica todos os arquivos pendentes em ordem, inclusive os contratos de segurança e Realtime do commit atual, e `db:check` valida o contrato esperado. A presença dos arquivos no Git não comprova aplicação no Supabase remoto: confirme backup, histórico e schema check durante o procedimento controlado, sem inferir que o deploy já ocorreu.
...
| `src/backend`            | Auth, consultas/RPC, Storage, Realtime, webhooks e funções de domínio compartilhadas entre Edge e fallback Node. |
| `src/server`             | Handler BFF, pedidos, pagamentos, assinaturas, entrega, ledger e servidor HTTP.                                  |
...
- O navegador usa `/api/backend` e `/storage` no próprio domínio. Para pedidos em tempo real, abre WSS somente no host exato do Supabase com chave publicável e ticket privado de curta duração; credenciais administrativas permanecem no servidor.
- `EDGE_PROXY_SECRET` existe apenas nos secrets da Edge e no snippet Nginx `root:root:0600`; chamadas diretas às Functions são rejeitadas.
...
- [Migração do BFF para Supabase Edge](supabase/EDGE_MIGRATION.md)
